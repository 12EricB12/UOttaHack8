...
public:
    ...
    /**
     * Set callback invoked on EVERY status code update (regardless of whether it changed).
     * This is useful for camera tuning which needs to observe all status updates.
     */
    ...
    absl::Status SetOnEdgeMetricsOutput(
        const std::function<absl::Status(const physiology::Metrics&, int64_t input_timestamp)>& on_edge_metrics_output
    );
...
// == fixed/static after initialization
    ...
    // if needed, set to a callback that handles ALL status code updates (not just changes)
    ...
    std::function<absl::Status(const physiology::Metrics&, int64_t input_timestamp)> OnEdgeMetricsOutput =
        [](const physiology::Metrics&, int64_t input_timestamp) { return absl::OkStatus(); };
